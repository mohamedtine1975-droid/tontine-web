...
export default function Dashboard() {
  ...
  const openPaymentLink = (method: "wave" | "orange") => {
    ...
    if (method === "wave") {
      ...
      const omNumber = settings.adminOmNumber.replace(/\s/g, "");
      window.open(`tel:${omNumber}`, "_blank");
      toast("Composez le numéro et effectuez votre transfert Orange Money", { icon: "🟠" });
    }
  };

  const handleSubmitPayment = async () => {
    if (!user || !userData || !settings) return;
    if (!screenshot) { toast.error("Veuillez joindre une capture d'écran de la transaction"); return; }
    setPaying(true);
    try {
      // Upload screenshot
      const storageRef = ref(storage, `screenshots/${user.uid}/${currentMonth}_${Date.now()}.jpg`);
        ...
        <div className="card" style={{ padding: "1.5rem", marginBottom: "1.5rem", borderLeft: `4px solid ${thisMonthPayment?.status === "validated" ? "#1A6B35" : thisMonthPayment?.status === "pending" ? "#8B6914" : "#B04A10"}` }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: "0.75rem" }}>
              ...
              {thisMonthPayment?.status === "validated" && (
                <span className="badge-paid">✅ Payé et validé</span>
              )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(26,58,42,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "1rem" }}>
          <div className="card" style={{ maxWidth: 460, width: "100%", padding: "2rem", maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <div style={{ background: "var(--cream)", borderRadius: "12px", padding: "1.25rem", marginBottom: "1.25rem" }}>
              <p style={{ fontWeight: 600, fontSize: "0.9rem", color: "var(--forest)", marginBottom: "0.5rem" }}>
                Étape 1 — Effectuer le transfert
              </p>
              <p style={{ color: "var(--text-muted)", fontSize: "0.82rem", marginBottom: "1rem" }}>
                Cliquez pour ouvrir {payMethod === "wave" ? "Wave" : "Orange Money"} et envoyer {settings.monthlyAmount.toLocaleString("fr-FR")} FCFA au numéro de l'admin.
              </p>
              <button className="btn-gold" onClick={() => openPaymentLink(payMethod)} style={{ width: "100%", fontSize: "0.9rem" }}>
                {payMethod === "wave" ? "🌊 Payer via Wave" : "🟠 Payer via Orange Money"}
              </button>
            </div>
            ...
            <div style={{ marginBottom: "1.25rem" }}>
              ...
              <label style={{ display: "block", border: "2px dashed #e0d8c0", borderRadius: "12px", padding: "1.5rem", textAlign: "center", cursor: "pointer", background: screenshot ? "#E6F4EC" : "white", transition: "all 0.2s" }}>
                <input type="file" accept="image/*" style={{ display: "none" }} onChange={e => setScreenshot(e.target.files?.[0] || null)} />
                {screenshot ? (
                  <span style={{ color: "#1A6B35", fontWeight: 600 }}>✅ {screenshot.name}</span>
                ) : (
